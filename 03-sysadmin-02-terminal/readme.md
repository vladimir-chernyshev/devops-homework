...
---
  ...
  В традиционных Unix-like операционных системах сразу после запуска и загрузки ядра операционной системы ядро, как последнюю стадию инициализации операционной системы, запускает первый пользовательский процесс **init**, который продолжает загрузку операционной системы уже в пользовательском режиме и является прародителем всех пользовательских процессов в системе. Сам процесс запуска операционной системы немного отличается для BSD и Linux-систем и назывался соответственно [rc](http://man.openbsd.org/rc) и SysV init. (Оговорюсь тут же в скобках, что я BSD-шник). Начиная с релиза [15.04](https://jaxenter.com/ubuntu-switching-systemd-115290.html), Ubuntu переключилась на систему инициализации [systemd(1)](http://0pointer.de/blog/projects/systemd.html), придуманной сотрудником RedHat Леонардом Потерингом, - иными словами, первым процессом, запускаемым ядром, является демон **systemd**, который с целью обратной совместимости с SysV, называет себя **init**. [Вот тут](https://askubuntu.com/questions/893816/inconsistent-output-for-init-process-name-using-ps) любопытное разъяснение, подтверждаемое запуском команд на виртуальной машине:
        ...
        root           1       0  0 41835 11380   0 12:51 ?        00:00:05 /sbin/init
